"""
PI Advisor MCP Server — ADN (Air-gapped Deployment / WiNdows) edition.

Transport is selected via MCP_TRANSPORT:
  stdio — launched as a subprocess by a local MCP client (dev/testing).
  http  — MCP streamable HTTP endpoint at /mcp (required for Microsoft
          Copilot Studio integration), protected by auth.AuthMiddleware
          (AUTH_MODE = none | api_key | entra_jwt).
...
Tool catalogue (16 tools):
  ...
  Calculation:
    run_calculation            — restricted pandas/numpy calculations on fetched data
...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "For any non-trivial arithmetic on retrieved data, use run_calculation "
        "instead of computing values yourself."
    ),
)
...
async def run_calculation(
    ...
    """
    Execute a restricted Python calculation on data you have already fetched.

    Use this for numerical work that would be unreliable to do mentally:
    rolling averages, % deviation from target, unit conversions, aggregations
    across many tags, correlations, min/max windows, mass/energy balances.

    Environment (pre-imported, nothing else can be imported):
      np (numpy), pd (pandas), math, statistics, json, datetime
    The JSON passed via data_json is available as the variable `data`.
    Assign your answer to `result` (or make it the last expression).

    Not allowed: import statements, file/network/OS access, dunder access.
    Execution time limit applies (default 10 s).

    Parameters
    ----------
    code      : str   Python code. Example:
                      "df = pd.DataFrame(data['values']); result = df['Value'].mean()"
    data_json : str   Optional JSON string with input data (e.g. the 'values'
                      array returned by get_recorded_values).

    Returns {"result": ..., "printed": [...]} or {"error": "..."}.
    """
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, run_calculation_sync, code, data_json)
...
def pi_query_guide() -> str:
    ...
    return """
You are a PI System expert assistant for upstream oil & gas operations.
...
6. **For numerical analysis** (averages, % deviation, unit conversion, trends):
   fetch the data first, then use run_calculation — never do arithmetic mentally.
...
async def _run_http() -> None:
    ...
    # Fail fast on unusable auth configuration.
    init_auth()
    if config.auth.mode == "none" and not os.getenv("ALLOW_UNAUTHENTICATED_HTTP"):
        logger.critical(
            "AUTH_MODE=none is not allowed with MCP_TRANSPORT=http. "
            "Set AUTH_MODE=api_key or entra_jwt (or, for a local smoke test "
            "only, set ALLOW_UNAUTHENTICATED_HTTP=1)."
        )
        sys.exit(1)

    await startup()

    # FastMCP's streamable-HTTP ASGI app (endpoint: /mcp), wrapped with the
    # inbound auth middleware. Wrapping preserves the app's lifespan events.
    ...
    if config.server.ssl_certfile and config.server.ssl_keyfile:
        ...
        logger.info("Serving HTTPS directly (cert: %s)", config.server.ssl_certfile)
    else:
        logger.warning(
            "No MCP_SSL_CERTFILE/MCP_SSL_KEYFILE set — serving plain HTTP. "
            "Terminate TLS with a reverse proxy (IIS/ARR) in front, or set "
            "the cert paths. Copilot Studio requires HTTPS end-to-end."
        )
    ...
    logger.info(
        "MCP streamable HTTP endpoint: %s://%s:%d/mcp (auth: %s)",
        "https" if ssl_kwargs else "http",
        config.server.host, config.server.port, config.auth.mode,
    )
    try:
        await server.serve()
    finally:
...
if __name__ == "__main__":
    ...
    try:
        if transport == "http":
            ...
            logger.critical("Unknown MCP_TRANSPORT: %r (expected stdio | http)", transport)
            sys.exit(1)
